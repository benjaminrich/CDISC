...
read_data_sas <- function(file, path=NULL, ...) {
    ...
    if (grepl("\\.xpt$", file)) {
        x <- tryCatch({
            ...
            haven::read_xpt(file, ...)   # Requires package 'haven'
        }, error=function(e) e)
        if (inherits(x, "error")) {
            # Give up
...
#' Canonical representation of a `data.frame`
#'
#' The canonical representation of a `data.frame` makes it easier to merge data
#' from different SDTM domains. Specifically, there is a CDISC convention that
#' dictates the many fields in SDTM data sets have the domain abbreviation as
#' column name prefix (e.g. lbtest, lbdtc, etc). If you want to merge these
